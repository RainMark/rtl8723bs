		...
		if (sr != 0) {
			...
			if (request->n_ssids == 1 && request->n_channels == 1) {
				...
				if (ssids[0].ssid_len == 0)
					;
				else if (pssid->SsidLength == ssids[0].ssid_len && !memcmp(pssid->Ssid, ssids[0].ssid, ssids[0].ssid_len))
...
static int rtw_cfg80211_set_encryption(struct net_device *dev, struct ieee_param *param, u32 param_len)
{
	...
	if (param->sta_addr[0] == 0xff && param->sta_addr[1] == 0xff &&
	    param->sta_addr[2] == 0xff && param->sta_addr[3] == 0xff &&
	    param->sta_addr[4] == 0xff && param->sta_addr[5] == 0xff) {
		if (param->u.crypt.idx >= WEP_KEYS && param->u.crypt.idx > BIP_MAX_KEYID) {
			ret = -EINVAL;
			goto exit;
		}
	} else {
	...
	if (padapter->securitypriv.dot11AuthAlgrthm == dot11AuthAlgrthm_8021X) {
		...
		if (check_fwstate(pmlmepriv, WIFI_STATION_STATE | WIFI_MP_STATE) == true) {
			...
			pbcmc_sta = rtw_get_bcmc_stainfo(padapter);
			if (pbcmc_sta == NULL) {
				DBG_8192C("Set OID_802_11_ADD_KEY: bcmc stainfo is null\n");
			} else {
				/* Jeff: don't disable ieee8021x_blocked while clearing key */
				if (strcmp(param->u.crypt.alg, "none") != 0)
					pbcmc_sta->ieee8021x_blocked = false;
	...
	struct net_device *ndev, u8 key_index, bool unicast, bool multicast)
{
	...
	DBG_871X(FUNC_NDEV_FMT "key_index =%d, unicast =%d, multicast =%d.\n",
		 UNC_NDEV_ARG(ndev), key_index, unicast, multicast);

	/* set wep default key */
	if ((key_index < WEP_KEYS) && ((psecuritypriv->dot11PrivacyAlgrthm == _WEP40_)
				       || (psecuritypriv->dot11PrivacyAlgrthm == _WEP104_))) {
		psecuritypriv->ndisencryptstatus = Ndis802_11Encryption1Enabled;
