			 ...
			 union iwreq_data *wrqu, char *extra)
{
	...
	if ((true == padapter->bDriverStopped) || (true == padapter->bSurpriseRemoved) || (pdata == NULL)) {
		ret = -EINVAL;
		goto exit;
	}
	...
	case 0x7F:
		...
		case 0x1b:
		{
			...
			if (arg == 0) {
				...
				/*  max_rx_rate < MSC0 -> B or G -> disable HT */
				if (max_rx_rate < 0xc) {
...
static int rtw_ioctl_wext_private(struct net_device *dev, union iwreq_data *wrq_data)
{
	...
	k = -1;
	while ((++k < num_priv_args) &&
	       strcmp(priv_args[k].name, cmdname))
		;
